...
## v2 — Step 10 (Message List = UNION of all sheets)
- **Fixes a real bug:** the single-sheet pick (even after case-insensitive matching) preferred "(FD+HS) all CAN" and silently dropped every mapping that exists only on "all PDU" — including all `Frame Container` (CoC) linkage. `LoadMessageList(IXLWorkbook)` now **combines every Message-List worksheet** (`all PDU`/`fd+hs`/`all CAN`/`message set`/`message list` with a `Signal Name` column; the `container`/`construction` CoC sheet is excluded and loaded separately). The per-sheet parse is extracted into `LoadSignalsFromSheet` (tags `SignalDef.SourceSheet`).
- **Union rules:** sheets load in priority order (**"all PDU" first** so its container-linked rows win); on a duplicate key (Signal·Frame·Contained-I-PDU·Byte·Bit, OrdinalIgnoreCase) the winner is kept and only **blank string/null-int fields are backfilled** + the per-ECU **T/R marks unioned** — non-blank data is never overwritten. Exact in-sheet duplicates collapse (T/R merged, nothing lost). Per-sheet "+N new / M overlap" + combined total (incl. Frame-Container count) are reported via progress + `Debug.WriteLine`.

## v2 — Step 9 (primary source → "Message List all PDU")
- **Primary sheet switched** to the superset **"Message List all PDU"** (one row per signal/PDU/frame), falling back to "(FD+HS) all CAN" only if absent. All columns mapped strictly by header via `ColumnMap`; the ECU T/R auto-detection re-runs on this sheet (its 39 node columns incl. `PIU_MASTER`). The unmapped frame/contained-PDU position and `Optional Frame`/`Protocol` columns are numeric/text so they're never mistaken for T/R nodes.
- **Proper columns now used:** `Byte Position (0-7)` / `Bit Position (7-0)` (frame-absolute — better layout for containers), `Frame Size`, **`Unavailable Value (Bin/Hex)`** (the AT compare now checks ISR `ReqUnavailableValue` against this, not Coding), `Coding (Bin/Hex)` / `Meaning` (2^N + magic checks), `Functional`, `Event`, and **`Frame Container`** for the frame↔container linkage.
- **`Frame Container` drives container resolution directly** — `FrameTraceService.ResolveContainer` now synthesizes a minimal container (master Tx = T-marked ECU on the container frame; secured = `*SC_FD`) straight from the column when the Construction sheet has no record; this feeds the route check, trace, container decision and frame-layout view.
- **Construction-of-Container kept only for provenance** the superset doesn't carry as columns — `original Tx unit` (source ECU), `original ID`, explicit `MAC` — and the container bands. *(Assumption: those three fields still warrant the Construction load; everything else is derived from "all PDU".)*
- **Counts printed** on load (`Summary` now reports signals/distinct/multi-frame, **frames, PDUs, ISR-linked signals**, demands, applied ISRs, containers, dico, routes) + a `Debug.WriteLine` — to verify nothing was lost in the switch.
